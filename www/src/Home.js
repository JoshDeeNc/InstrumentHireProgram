...
function Home({ toDos, deleteToDo, completeToDo }) {
  ...
  function search(records) {
    ...
    if (filtDates.length > 0 && dtRange) {
      ...
      return records.filter(item => new Date(new Date(item.creation_date).toLocaleDateString()) >= new Date(startEnd[0]) && new Date(new Date(item.creation_date).toLocaleDateString()) <= new Date(startEnd[1]))
    }
    return records
  }
